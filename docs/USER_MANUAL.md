...
## 5. Micronet Live (TUI) — full guide
...
- `d`: cycle message delay/latency (0t -> 2t -> 5t -> 10t -> 0t)
...
- `propose <kind> <payload...>` consumes the rest of the line as payload, so payload may contain spaces.
- `history [n]` prints recent commands.
- `events [n]` prints recent runtime events generated by applying messages.
- `export-state` prints a readable snapshot of the current local state.
- `replay [n]` rebuilds the runtime and replays the last n messages from the in-process message log.
- `save-log <path>` saves the in-process message log to a file.
- `load-log <path>` loads a message log from a file and replays it.
